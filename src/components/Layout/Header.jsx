      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="flex items-center space-x-2">
            <img src={RoririLogo} alt="Roriri Logo" className="h-10 w-auto" />
          </Link>
            ...
            {navigation.map((item) => (
              <div key={item.name} className="relative" ref={item.dropdown ? dropdownRef : null}>
                {item.dropdown ? (
                  <>
                    <button
                      onClick={() => setIsBusinessesOpen((prev) => !prev)}
                ...
                <div key={item.name}>
                  {item.dropdown ? (
                    <>
                      <button
                        onClick={() => setIsBusinessesOpen((prev) => !prev)}
                        className="px-3 py-2 text-gray-900 font-medium flex justify-between items-center w-full"
                      >
                        {item.name}
                        <ChevronDown className={`ml-2 h-4 w-4 transition-transform ${isBusinessesOpen ? 'rotate-180' : ''}`} />
                      </button>
                      {isBusinessesOpen &&
                        item.dropdown.map((dropdownItem) => (
                          <Link
